...
export function validateAcademic(state) {
  const { type_student, level, institution, study_area, planned_graduation } =
    state;
...
export function errorAcademic(error) {
  ...
  for (let propiedad in errorAcademicState) {
    if (
      errorAcademicState[propiedad] !== "" &&
      errorAcademicState[propiedad] !== undefined
    )
      return false;
  }
  return true;
...
export function errorInterest(error) {
  ...
  for (let propiedad in errorInterestState) {
    if (
      errorInterestState[propiedad] !== "" &&
      errorInterestState[propiedad] !== undefined
    )
      return false;
  }
  return true;
...
export function errorExtra(error) {
  ...
  for (let propiedad in errorExtraState) {
    if (
      errorExtraState[propiedad] !== "" &&
      errorExtraState[propiedad] !== undefined
    )
      return false;
  }
  return true;
